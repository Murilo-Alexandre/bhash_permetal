...
REM ============================================
REM BHASH - Bootstrap PC novo (auto-elevado)
REM Edite apenas estas variaveis, se necessario:
REM ============================================
...
REM Eleva para Administrador automaticamente (UAC)
net session >nul 2>&1
if not "%errorlevel%"=="0" (
...
echo.
echo [1/5] Configurando hosts necessarios...
if "%SKIP_HOSTS%"=="1" (
...
echo.
echo [2/5] Baixando instalador de bootstrap...
...
set "EXTRA_ARGS="
if "%SKIP_HOSTS%"=="1" set "EXTRA_ARGS=%EXTRA_ARGS% -SkipHosts"
if "%ALLOW_INSECURE_TLS%"=="1" set "EXTRA_ARGS=%EXTRA_ARGS% -AllowInsecureTls"

echo.
echo [3/5] Instalando BHash + updater sem senha...
powershell -NoProfile -ExecutionPolicy Bypass -File "%BOOTSTRAP_PS%" -ServerIp "%SERVER_IP%" -UpdateUrl "%UPDATE_BASE%" %EXTRA_ARGS%
if errorlevel 1 goto :fail

echo.
echo [4/5] Conferindo estado final do updater...
powershell -NoProfile -ExecutionPolicy Bypass -Command "if (Test-Path 'C:\ProgramData\BHashChat\Updater\state.json') { Get-Content 'C:\ProgramData\BHashChat\Updater\state.json' } else { Write-Host '[AVISO] state.json ainda nao encontrado.' }"

echo.
echo [5/5] Concluido com sucesso.
echo BHash instalado e preparado para update automatico sem senha por usuario.
pause
exit /b 0

:fail
echo.
echo [ERRO] Nao foi possivel concluir a instalacao automatica.
